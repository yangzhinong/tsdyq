            ...
            var dlg = new BootstrapDialog({
                ...
                message: function () {
                    ...
                    $div.find('#btn-download').click(function () {
                        ...
                        if (p == "") {
                            dialogAlert("必须选择一个Project后,才能下载!", LayerIcon.Error);
                            return;
                        }
                    ...
                    $div.find('#btn-del-a-project').click(function () {
                        ...
                        if (p == "") {
                            Loading(false);
                            dialogAlert("必须选择一个Project后,才能删除!", LayerIcon.Error);
                            return;
                        }

                        $.post("/NPSAInputData/DelAProject", { project: p }, function (data: IJsonMsg) {
                            if (data.code) {
                                Loading(false);
                                dialogMsg(data.msg, LayerIcon.OK);
                            } else {
                                Loading(false);
                                dialogAlert(data.msg, LayerIcon.Error);
                            }
                        });
                    })
                    ...
                    $div.find("#frm-upload").ajaxForm({
                        success: function (data: IJsonMsg) {
                            Loading(false);
                            if (data.code) {
                                dialogAlert(data.msg, LayerIcon.OK);
